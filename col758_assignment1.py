...
def RandomizedTransitiveClosure(graph, epsilon, n, choice_of_estimator):
  ...
  w = np.zeros((n, k))
  for i in range(k):
    # Assign to each v ∈ V a uniformly independent random label from [0, 1]
...
def TransitiveClosure(graph, n):
  ...
  return (np.sum(tc, axis = 1), end - begin)
...
for epsilon in epsilons:
  ...
  for i in range(noTest):
    n = n_inp[i]
    G = Gr[i]
    (tc, t1) = TransitiveClosure(G, n)
    (P, t2) = RandomizedTransitiveClosure(G, epsilon, n, 0)
  ...
  time1.append(time_tc)
  time2.append(time_random)

X_axis = np.arange(len(n_inp))
plt.bar(X_axis-0.2, p1_obs1[0], 0.2,label = "epsilon = 0.001")
plt.bar(X_axis, p1_obs1[1], 0.2, label = "epsilon = 0.05")
plt.bar(X_axis+0.2, p1_obs1[2], 0.2, label = "epsilon = 0.1")
plt.xlabel('number of node (n)')
plt.ylabel('mean_squared_error')
plt.xticks(X_axis, n_inp)
plt.legend()
plt.show()

X_axis = np.arange(len(n_inp))
plt.bar(X_axis-0.2, p1_obs2[0], 0.2,label = "epsilon = 0.001")
plt.bar(X_axis, p1_obs2[1], 0.2, label = "epsilon = 0.05")
plt.bar(X_axis+0.2, p1_obs2[2], 0.2, label = "epsilon = 0.1")
plt.xlabel('number of node (n)')
...
X_axis = np.arange(len(n_inp))
plt.bar(X_axis-0.2, time2[0], 0.2,label = "epsilon = 0.001")
plt.bar(X_axis, time2[1], 0.2,label = "epsilon = 0.05")
plt.bar(X_axis+0.2, time2[2], 0.2,label = "epsilon = 0.1")
plt.xlabel('number of node (n)')
...
plt.plot(prob_x[0], prob_obs2[0], label = "n = 50")
...
plt.plot(prob_x[0], prob_obs1[0], label = "n = 50")
